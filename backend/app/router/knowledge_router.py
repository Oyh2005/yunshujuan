...
async def serve_knowledge_image(
    ...
    """
    # 安全校验（防路径穿越）：
    # 1. md5 必须是 32 位十六进制（参与目录拼接）
    # 2. filename 必须是纯文件名，不得包含路径分隔符（. / \ .. 等）
    import re
...
async def clip_webpage(
    ...
    """
    网页剪藏：抓取 URL → 提取正文 → 走切片/向量化管线入库。
    安全：仅 http/https + 拒绝内网地址（防 SSRF）+ 超时 + 大小限制。
    """
    result = await knowledge_service.handle_clip(payload.url.strip(), user_id)
    return success_response(message=f"剪藏成功：{result['filename']}", data=result)
