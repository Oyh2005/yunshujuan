...
// 虚拟滚动窗口：单次最多渲染的消息条数（超长会话时只渲染最近部分，顶部可加载更早）
...
export default function AIChat() {
  ...
  // 渲染窗口起始下标：默认显示最近 RENDER_WINDOW 条，顶部可加载更早
  const [visibleStart, setVisibleStart] = useState(0)
  const prevLenRef = useRef(0)

  // 消息增多时：若窗口原本在底部则跟随滚动；用户已上翻查看早期消息时不打扰
  useEffect(() => {
    const prevLen = prevLenRef.current
    prevLenRef.current = messages.length
    if (messages.length > prevLen && visibleStart + RENDER_WINDOW >= prevLen) {
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6">
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          {visibleStart > 0 && (
            <div className="flex justify-center">
              <button
                onClick={() => setVisibleStart((s) => Math.max(0, s - RENDER_WINDOW))}
                className="px-4 py-1.5 text-xs rounded-md border border-[var(--color-border)] text-[var(--color-text-secondary)] hover:border-[var(--color-accent)] hover:text-[var(--color-accent)] transition-colors"
              >
                {t('chat.loadEarlier')}
              </button>
            </div>
          ...
          <ErrorBoundary
            fallback={
              <div className="py-8 text-center text-sm text-[var(--color-text-secondary)]">
                消息渲染出错，请<a href="/chat" className="text-[var(--color-accent)] underline">新建对话</a>或刷新页面
              </div>
            }
          >
          {messages.slice(visibleStart).map((msg, i) => {
            const absIndex = visibleStart + i
            return (
            <div key={absIndex} className={`msg-pop-in flex gap-3 ${msg.role === 'user' ? 'justify-end' : ''}`}>
              {msg.role === 'assistant' && (
              ...
              <div className={`max-w-[75%] ${msg.role === 'user' ? 'order-first' : ''}`}>
                {msg.role === 'user' ? (
                  <div className="px-4 py-2.5 rounded-2xl bg-[var(--color-accent)] text-[var(--color-accent-foreground)] text-sm">
                    {msg.content}
                  </div>
                ) : (
                  <>
                    {absIndex === messages.length - 1 && currentSteps.length > 0 && (
                      <div className="mb-3">
                    ...
                    <div className={`prose prose-sm max-w-none markdown-body${theme === 'dark' ? ' prose-invert' : ''}`}>
                      {/* 不使用 rehypeRaw：LLM 输出可能包含畸形/危险 HTML，渲染原始 HTML 会导致
                          React 渲染异常（应用闪退）并带来 XSS 风险，纯 Markdown 渲染足够安全 */}
                      <ReactMarkdown rehypePlugins={[rehypeHighlight]}>
                        {msg.content}
                      </ReactMarkdown>
                    </div>
                    {hasStreamingAssistant && absIndex === messages.length - 1 && (
      ...
      <div className="border-t border-[var(--color-border)] bg-[var(--color-card)] px-6 py-4">
        <div className="max-w-3xl mx-auto flex gap-3">
          ...
          <button
            ...
            className="flex items-center justify-center w-10 h-10 rounded-lg bg-[var(--color-accent)] text-[var(--color-accent-foreground)] hover:bg-[var(--color-accent-hover)] disabled:opacity-40 transition-colors shrink-0"
          >
