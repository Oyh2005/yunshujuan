...
export default function CategoryManageDialog({
  ...
  useEffect(() => {
    // 有意仅在打开时用最新 categories 初始化（categories 每次渲染为新数组，不能进依赖）
      ...
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-[var(--color-card)] rounded-lg shadow-xl p-6 w-[440px] max-w-[90vw] max-h-[80vh] flex flex-col">
        ...
        <div className="flex gap-2 mb-4">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-md bg-[var(--color-accent)] text-[var(--color-accent-foreground)] disabled:opacity-40 transition-opacity"
          >
            <Plus size={14} />
            新建
          </button>
