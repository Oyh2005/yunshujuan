...
// Custom rule: wiki-link span → [[标题]]（双链语法还原，保证保存的 Markdown 无损）
turndown.addRule('wikiLink', {
  filter: (node) =>
    node.nodeName === 'SPAN' &&
    (node as HTMLElement).classList.contains('wiki-link'),
  replacement: (content) => `[[${content.trim()}]]`,
})

/** Markdown → HTML，并将 [[标题]] 替换为 wiki-link 高亮 span（不拦截输入） */
function parseMarkdownWithWikiLinks(md: string): string {
  const html = String(marked.parse(md || ''))
  return html.replace(/\[\[([^[\]]+)\]\]/g, '<span class="wiki-link">$1</span>')
}
...
function EditorToolbar({ editor }: { editor: Editor | null }) {
  // Hooks 必须在任何条件 return 之前调用（React Hooks 规则）
...
const TiptapEditor = forwardRef<TiptapEditorHandle, TiptapEditorProps>(({ value, onChange, placeholder, onAutocomplete }, ref) => {
  ...
  const editor = useEditor({
    ...
    content: parseMarkdownWithWikiLinks(value),
    onUpdate: ({ editor }) => {
  ...
  useEffect(() => {
    ...
    editor.commands.setContent(parseMarkdownWithWikiLinks(value))
  ...
  useImperativeHandle(ref, () => ({
    scrollToHeading: (text: string, level: number) => {
      if (!editor) return
      // eslint-disable-next-line no-useless-escape -- 字符类内同时含 [ 与 ]，转义不可省略
